...
<div class="row">
    <div class="col-md-8">
        ...
        @if(!empty($post->image))
            <img src="{{ asset('images/'.$post->image) }}" height="300" width="750"/>	
        @endif
        ...
        <div class="well">
            <dl class="dl-horizontal">
                ...
                <p><a href="{{route('blog.single',$post->slug)}}">
                        {{route('blog.single',$post->slug)}}</a></p>
            </dl>

            <dl class="dl-horizontal">
                <label>Category:</label>
                <p>{{ $post->category->name }}</p>
                <!--                one-to-many is why from $post can use category table-->
            </dl>

            <dl class="dl-horizontal">
                <label>Created At:</label>
                <p>{{date('j M Y  G:i', strtotime($post->created_at))}}</p>
            </dl>

            <dl class="dl-horizontal">
                <label>Last Updated:</label>
                <p>{{date('j M Y  G:i', strtotime($post->updated_at))}}</p>
            </dl>
            <hr>
            <div class="row">
                <div class="col-sm-6">
